...
export const FarmerSyncDashboard = () => {
  ...
  // Background sync progress removed — all cumulative refresh is manual or post-sync
  ...
  useEffect(() => {
    ...
    // Listen for post-sync cumulative refresh trigger
    const handleCumulativeRefresh = () => {
      console.log('[SyncDash] cumulativeRefreshNeeded received — triggering refresh');
      loadData(true);
    };

    window.addEventListener('cumulativeRefreshNeeded', handleCumulativeRefresh);
    
    return () => { 
      cancelledRef.current = true; 
      window.removeEventListener('cumulativeRefreshNeeded', handleCumulativeRefresh);
    };
  }, [loadData]);
